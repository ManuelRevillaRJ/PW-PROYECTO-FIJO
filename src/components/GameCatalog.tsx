...
export default function GameCatalog({ gameList }: GameCatalogProps) {
    ...
    const maxPrice =
        gameList.length > 0
            ? Math.max(...gameList.map((g) => g.precio)) + 1
            : 100
    ...
    const filteredGames = useMemo(() => {
        return gameList.filter((game) => {
            ...
            if (filters.selectedCategorias.length > 0) {
                if (!filters.strict) {
                    if (
                        !filters.selectedCategorias.some((c) =>
            ...
            if (
                ...
                !filters.selectedPlataformas.some((p) =>
                    game.plataformas.includes(p)
                )
            ) {
                return false
            }
                        ...
                        <form className="mx-3">
                            ...
                            <div className="mb-4">
                                ...
                                <RangeSlider
                                    ...
                                    max={maxPrice}
                                    value={[
                                        filters.rangoPrecio[0],
                                        filters.rangoPrecio[1],
                                    ]}
                                    onInput={([min, max]) => {
                                        setFilters((prev) => ({
                                            ...prev,
